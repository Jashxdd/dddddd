...
export default {
  ...
  async execute(message) {
    ...
    if (mentionFormats.includes(trimmed)) {
      if (await isGloballyBlacklisted(message.author.id)) {
        if (mePermissions) {
          await message.reply({
            content:
              '⛔ Furmin hizmetlerine erişimin sınırlandırıldı. Ayrıntılar için bot sahibiyle iletişime geçmelisin.',
    ...
    if (detectedPrefix) {
      ...
      if (await isGloballyBlacklisted(message.author.id)) {
        await message.reply({
          content:
            '⛔ Furmin hizmetlerine erişimin sınırlandırıldı. Lütfen daha fazla bilgi için bot sahibine başvur.',
          allowedMentions: { repliedUser: false }
        });
        return;
      }
