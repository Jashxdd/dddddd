import { readdir } from 'node:fs/promises';
import { join, dirname, extname } from 'node:path';
import { fileURLToPath, pathToFileURL } from 'node:url';
import { config } from '../config.js';
import { getFeatureToggleKey, isFeatureEnabled } from './featureFlags.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
const commandsDirectory = join(__dirname, '..', 'prefix');

async function walk(directory) {
  const entries = await readdir(directory, { withFileTypes: true }).catch(() => []);
  const files = await Promise.all(
    entries.map(async (entry) => {
      const fullPath = join(directory, entry.name);
      if (entry.isDirectory()) {
        return walk(fullPath);
      }

      if (extname(entry.name) !== '.js') {
        return [];
      }

      return [fullPath];
    })
  );

  return files.flat();
}

const disabledPrefixCommands = new Set(config.disabledPrefixCommands ?? []);

export async function loadPrefixCommands() {
  const commandFiles = await walk(commandsDirectory);
  const commands = [];
  const seenNames = new Set();
  let skippedForDisable = 0;

  for (const file of commandFiles) {
    const fileUrl = pathToFileURL(file).href;
    const imported = await import(fileUrl);
    const exported = imported.default ?? imported;

    const commandSeeds = [];

    if (Array.isArray(exported)) {
      commandSeeds.push(...exported);
    } else if (exported && Array.isArray(exported.commands)) {
      commandSeeds.push(...exported.commands);
    } else {
      commandSeeds.push(exported);
    }

    if (!commandSeeds.length) {
      console.warn(`⚠️ ${file} dosyasından prefix komutu yüklenemedi.`);
      continue;
    }

    for (const seed of commandSeeds) {
      if (!seed || typeof seed !== 'object') {
        console.warn(`⚠️ ${file} dosyasında geçersiz komut bildirimi atlandı.`);
        continue;
      }

      if (!seed.name || typeof seed.execute !== 'function') {
        console.warn(`⚠️ ${file} dosyasında geçerli bir prefix komutu bulunamadı.`);
        continue;
      }

      const name = String(seed.name).trim().toLowerCase();
      if (!name) {
        console.warn(`⚠️ ${file} dosyasında geçerli bir komut adı bulunamadı.`);
        continue;
      }

      if (seenNames.has(name)) {
        console.warn(`⚠️ ${name} adına sahip birden fazla prefix komutu bulundu. ${file} atlandı.`);
        continue;
      }

      if (disabledPrefixCommands.has(name)) {
        console.log(`ℹ️  ${name} önek komutu yapılandırma tarafından devre dışı bırakıldığı için yüklenmedi.`);
        skippedForDisable += 1;
        continue;
      }

      const featureKey = getFeatureToggleKey(seed);
      if (featureKey && !isFeatureEnabled(featureKey)) {
        console.log(
          `ℹ️  ${name} önek komutu "${featureKey}" özelliği kapalı olduğu için yüklenmedi.`
        );
        continue;
      }

      seed.name = name;
      seed.aliases = Array.isArray(seed.aliases)
        ? seed.aliases.map((alias) => String(alias).trim().toLowerCase()).filter(Boolean)
        : [];

      seenNames.add(name);
      commands.push(seed);
    }
  }

  if (skippedForDisable > 0) {
    console.log(`🔧 Yapılandırma ${skippedForDisable} önek komutunu devre dışı bıraktı.`);
  }

  return commands;
}
