...
function buildMarketEmbed(formatAmount, currencyName, category = 'all') {
  ...
  if (normalised !== 'all') {
    ...
    embed.setDescription('Satın alabileceğin özel eşyalar ve etkileri:');
  }
  ...
  if (normalised !== 'all') {
    const extraItems = economyItems
      .filter((item) => (item.category ?? 'genel').toLowerCase() !== normalised)
      .slice(0, 3);
    if (extraItems.length) {
      embed.addFields({
        name: 'Diğer kategorilerden öneriler',
        value: extraItems.map((item) => `${item.name} — ${formatAmount(item.price)}`).join('\n'),
        inline: false
      });
    }
  }
...
function registerCategoryChoices(option) {
  option.addChoices({ name: 'Tüm Ürünler', value: 'all' });
  MARKET_CATEGORIES.slice(0, 24).forEach((category) => {
    const label = category.charAt(0).toLocaleUpperCase('tr') + category.slice(1);
    option.addChoices({ name: label, value: category });
  });
  return option;
}
...
const data = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) => sub.setName('cark').setDescription('Çarkı çevirerek sürpriz ödüller kazanmayı dene.'))
  .addSubcommand((sub) => sub.setName('arena').setDescription('Furmin arenasında riskli bir mücadeleye gir.'))
  .addSubcommand((sub) =>
    sub
      .setName('yatirim')
  ...
  .addSubcommand((sub) =>
    sub
      .setName('market')
      .setDescription('Satın alabileceğin market ürünlerini listeler.')
      .addStringOption((option) =>
        registerCategoryChoices(option.setName('kategori').setDescription('Belirli bir kategoriye göre filtrele.'))
      )
  )
  .addSubcommand((sub) =>
    sub
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (subcommand === 'arena') {
        ...
        await interaction.editReply({ content: `⚔️ ${lines.join('\n')}` });
        return;
      }
      ...
      if (subcommand === 'market') {
        const categoryOption = interaction.options.getString('kategori') ?? 'all';
        const embed = buildMarketEmbed(formatAmount, currencyName, categoryOption);
        await interaction.editReply({ embeds: [embed] });
        return;
      }
